...
class TpModelWorker:
    ...
    def __init__(
        ...
        self.acc_time_with_waiting = 0
        self.acc_time_without_waiting = 0
    ...
    def forward_thread_func_(self):
        with torch.cuda.stream(self.forward_stream):
            while True:
                ...
                self.event_map[model_worker_batch.bid].set()
                self.acc_time_with_waiting += time.time() - tic1
    ...
    def resolve_future_token_ids(self, bid, future_obj):
        # logger.info(f"resolve future {bid=}")
        self.event_map[bid].wait()
        return self.future_token_ids_output[bid]
